...
SUBPROJECT_ROOT = ROOT / "3D-GNN-over-antibody-antigen"
...
class InterfaceGNN(nn.Module):
    ...
    def forward(
        ...
        # Clamp attributes/weights to keep activations bounded
...
def train_epoch(
    ...
    for graph in graphs:
        optimizer.zero_grad()
        pred = model(graph.x, graph.edge_index, graph.edge_weight, graph.edge_attr)
...
def evaluate(
    ...
    with torch.no_grad():
        for graph in graphs:
            output = model(graph.x, graph.edge_index, graph.edge_weight, graph.edge_attr)
...
def overfit_debug_run(
    ...
    subset = dataset.get_split("train")[:n_samples] or dataset.graphs[:n_samples]
    if not subset:
...
def main(argv: list[str] | None = None) -> None:
    ...
    train_targets_arr = np.array([float(g.y.item()) for g in dataset.get_split("train")])
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, grad_stats = train_epoch(
            model, dataset.get_split("train"), optimizer, loss_fn, track_grads=True
        )
